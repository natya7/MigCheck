...
public class MySqlRepairTemplates implements RepairTemplates {
    ...
    public String restoreColumnValues(String schema, String table, String park,
        ...
        return "UPDATE " + qualified(schema, table) + " t JOIN " + qualified(schema, park)
                + " p ON " + joinCondition(pkNames)
                + " SET t." + q(column) + " = p." + q(column) + ";\n"
                + "DROP TABLE " + qualified(schema, park) + ";";
    }

    @Override
    public String parkTableByRename(String schema, String table, String park) {
        return "RENAME TABLE " + qualified(schema, table) + " TO " + qualified(schema, park) + ";";
    }

    @Override
    public String restoreTableByRename(String schema, String table, String park) {
        return "DROP TABLE " + qualified(schema, table) + ";\n"
                + "RENAME TABLE " + qualified(schema, park) + " TO " + qualified(schema, table) + ";";
    }
